...
// Resize player
var resizePlayer = function(iframes, ratio) {
  if (!iframes[0]) return;
  ...
  iframes.each(function() {
    var current = $(this);
    if (width / rasio < height) {
      playerWidth = Math.ceil(height * rasio);
      current
        .width(playerWidth)
        .height(height)
        .css({ left: (width - playerWidth) / 2, top: 0 });
    } else {
      playerHeight = Math.ceil(width / rasio);
      current
        .width(width)
        .height(playerHeight)
        .css({ left: 0, top: (height - playerHeight) / 2 });
    }
  });
};
(function($) {
